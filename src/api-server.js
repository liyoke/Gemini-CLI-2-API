...
async function initializeConfig(
  ...
  const envFiles = [".env.local", ".env.dev", ".env.prod", ".env"];
  for (const file of envFiles) {
    if (fs.existsSync(file)) {
      dotenv.config({ path: file });
      console.log(`[Config] Loaded environment variables from ${file}`);
      break;
    }
  }
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--api-key") {
      ...
      if (i + 1 < args.length) {
        ...
        if (mode === "console" || mode === "file") {
          ...
          console.warn(
            `[Config Warning] Invalid mode for --log-prompts. Expected 'console' or 'file'. Prompt logging is disabled.`,
          );
        }
        i++;
      } else {
      ...
      if (i + 1 < args.length) {
        ...
        console.warn(
          `[Config Warning] --model-provider flag requires a value.`,
        );
      }
      ...
      if (i + 1 < args.length) {
        ...
        console.warn(
          `[Config Warning] --openai-api-key flag requires a value.`,
        );
      }
    ...
    // Gemini-specific arguments
      ...
      if (i + 1 < args.length) {
        ...
        console.warn(
          `[Config Warning] --gemini-oauth-creds-base64 flag requires a value.`,
        );
      }
      ...
      if (i + 1 < args.length) {
        ...
        console.warn(
          `[Config Warning] --gemini-oauth-creds-file flag requires a value.`,
        );
      }
      ...
      if (i + 1 < args.length) {
        ...
        console.warn(
          `[Config Warning] --system-prompt-file flag requires a value.`,
        );
      }
      ...
      if (i + 1 < args.length) {
        ...
        if (mode === "overwrite" || mode === "append") {
          ...
          console.warn(
            `[Config Warning] Invalid mode for --system-prompt-mode. Expected 'overwrite' or 'append'. Using default 'overwrite'.`,
          );
        }
        i++;
      } else {
        console.warn(
          `[Config Warning] --system-prompt-mode flag requires a value.`,
        );
      }
    } else if (args[i] === "--host") {
      if (i + 1 < args.length) {
  ...
  currentConfig.SYSTEM_PROMPT_CONTENT = await getSystemPromptFileContent(
    currentConfig.SYSTEM_PROMPT_FILE_PATH,
  );
...
async function getSystemPromptFileContent(filePath) {
  try {
    ...
    if (error.code === "ENOENT") {
      console.warn(
        `[System Prompt] Specified system prompt file not found: ${filePath}`,
      );
    } else {
      console.error(
        `[System Prompt] Error accessing system prompt file ${filePath}: ${error.message}`,
      );
    }
    return null;
  }
  ...
  try {
    ...
    console.error(
      `[System Prompt] Error reading system prompt file ${filePath}: ${error.message}`,
    );
    return null;
  }
}
...
async function initApiService(config) {
  ...
  for (const provider of providers) {
    try {
      ...
      console.warn(
        `[Initialization Warning] Failed to initialize service adapter for ${provider}: ${error.message}`,
      );
    }
  }
}
...
function createRequestHandler(config) {
  return async function requestHandler(req, res) {
    ...
    if (modelProviderHeader) {
      ...
      console.log(
        `[Config] MODEL_PROVIDER overridden by header to: ${currentConfig.MODEL_PROVIDER}`,
      );
    ...
    try {
      ...
      if (method === "GET") {
        if (path === "/v1/models") {
          return await handleModelListRequest(
            req,
            res,
            apiService,
      ...
      if (method === "POST") {
        ...
        const geminiUrlPattern = new RegExp(
...
async function startServer() {
  ...
  server.listen(CONFIG.SERVER_PORT, CONFIG.HOST, () => {
    ...
    console.log(
      `  Prompt Logging: ${CONFIG.PROMPT_LOG_MODE}${PROMPT_LOG_FILENAME ? ` (to ${PROMPT_LOG_FILENAME})` : ""}`,
    );
    console.log(`------------------------------------------`);
    console.log(
