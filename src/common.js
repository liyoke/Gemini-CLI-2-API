 ...
 * This is used to determine if two providers belong to the same underlying protocol (e.g., gemini, openai).
 * @param {string} provider - The model provider string (e.g., 'gemini-cli', 'openai-custom').
 * @returns {string} The protocol prefix (e.g., 'gemini', 'openai').
 */
...
export function isAuthorized(req, requestUrl, REQUIRED_API_KEY) {
  ...
  console.log(
    `[Auth] Unauthorized request denied. Bearer: "${authHeader ? "present" : "N/A"}", Query Key: "${queryKey}", x-goog-api-key: "${googApiKey}"`,
  );
  return false;
}
 ...
 * service, transforms them to the format expected by the client (OpenAI, Gemini, etc.),
...
export async function handleContentGenerationRequest(
  ...
  if (!fromProvider) {
    throw new Error(
      `Unsupported endpoint type for content generation: ${endpointType}`,
    );
  }
  ...
  const { model, isStream } = _extractModelAndStreamInfo(
    req,
    originalRequestBody,
    fromProvider,
  );
